...
# Browser-compatible H.264 output. NO AI-GENERATED IMAGES.
...
TEMP_AVI = OUT_DIR / "EMBOPSA_TEMP_MJPG.avi"
OUT_MP4 = OUT_DIR / "EMBOPSA_EARTH_ORBIT_500KYR_231KYR_H264.mp4"
...
nu = np.linspace(0.0, 2.0 * np.pi, ORBIT_SAMPLES, endpoint=True)

def orbit_xyz(a, e, inc, Omega, omega):
...
try:
    for j in range(N_FRAMES):
        ...
        ax.plot_surface(GX, GY, np.zeros_like(GX), alpha=0.05, color="white", linewidth=0)

        first = max(0, j - trail_step * trail_keep)
        for ti in range(first, j, trail_step):
            age = (j - ti) / max(1, trail_step * trail_keep)
            alpha = 0.08 + 0.28 * (1.0 - age)
            xh, yh, zh = orbit_xyz(a_f[ti], e_f[ti], inc_f[ti], Omega_f[ti], omega_f[ti])
            ax.plot(xh, yh, zh, color="#52D6FF", linewidth=0.8, alpha=alpha)

        x, y, z = orbit_xyz(a_f[j], e_f[j], inc_f[j], Omega_f[j], omega_f[j])
        ax.plot(x, y, z, color="#35E0A1", linewidth=2.2, alpha=0.98)
        ax.scatter([0], [0], [0], s=180, color="#FFD166", edgecolor="white", linewidth=0.8)

        nvec = orbit_normal(inc_f[j], Omega_f[j])
        ax.quiver(0, 0, 0, 0.75*nvec[0], 0.75*nvec[1], 0.75*nvec[2],
                  color="#FF9F43", linewidth=2.0, arrow_length_ratio=0.10)
        ...
        ax.set_title("Earth Orbital-Plane Evolution — La2010a",
        ...
        panel.text(0.00, 0.19, "Visible trail memory", color="#52D6FF", fontsize=12)
        panel.text(0.00, 0.145, "100,000 years", color="white", fontsize=16, fontweight="bold")
        panel.plot([0.00, 0.92], [0.06, 0.06], color="#28364A", linewidth=9, solid_capstyle="round")
        panel.plot([0.00, 0.92*phase], [0.06, 0.06], color="#35E0A1", linewidth=9, solid_capstyle="round")

        fig.text(0.035, 0.025,
                 "Actual La2010a osculating elements • fixed camera • orbit snapshots every 10 kyr",
...
print("\nEncoding browser-compatible H.264 MP4…")
...
if not OUT_MP4.exists() or OUT_MP4.stat().st_size < 100_000:
    raise RuntimeError("H.264 output was not created correctly.")

TEMP_AVI.unlink(missing_ok=True)
size_mb = OUT_MP4.stat().st_size / 1_000_000.0
print(f"Saved: {OUT_MP4}")
print(f"Size: {size_mb:.3f} MB | Duration: {DURATION_S} s | FPS: {FPS}")

display(HTML(f'''
<video width="960" controls autoplay muted playsinline preload="metadata" style="max-width:100%;background:#000">
  <source src="/files/{OUT_MP4}" type="video/mp4">
</video>
<p><b>H.264 file:</b> {OUT_MP4}</p>
'''))
