...
OUT_MP4 = OUT_DIR / "EMBOPSA_EARTH_ORBIT_500KYR_231KYR_CYCLE.mp4"
...
try:
    for j in range(N_FRAMES):
        ...
        GZ = np.zeros_like(GX)
        ax.plot_surface(GX, GY, GZ, alpha=0.05, color="white", linewidth=0)

        first = max(0, j - trail_step * trail_keep)
        trail_indices = list(range(first, j, trail_step))
        ...
        ax.quiver(
            0, 0, 0,
            0.75 * nvec[0], 0.75 * nvec[1], 0.75 * nvec[2],
        ...
        panel.text(0.00, 0.53, f"Inclination: {np.degrees(inc_f[j]):.6f}°",
                   color="white", fontsize=14)
        panel.text(0.00, 0.47, f"Eccentricity: {e_f[j]:.8f}",
                   color="white", fontsize=14)
        panel.text(0.00, 0.41, f"Semimajor axis: {a_f[j]:.9f} AU",
                   color="white", fontsize=14)
        ...
        panel.plot([0.00, 0.92 * phase], [0.06, 0.06], color="#35E0A1",
...
print(f"\nSaved: {OUT_MP4}")
print(f"Duration: {DURATION_S} s | FPS: {FPS} | Frames: {N_FRAMES}")
print(f"Trail: every {TRAIL_INTERVAL_KYR:.0f} kyr, last {TRAIL_MEMORY_KYR:.0f} kyr visible")
