# V0002
# Audit reference: quoted-TLIST resilient 6000-state JPL EMB acquisition
...
OUT = Path("/content/JPL_EARTH_ORBITAL_PRECESSION_FFT/EMBOPSA_V0002_OUTPUT")
CACHE = OUT / "cache_jpl_chunks"
STATE_CSV = OUT / "EMBOPSA_JPL_EMB_STATES_6000_V0002.csv"
NORMAL_CSV = OUT / "EMBOPSA_ORBITAL_NORMAL_6000_V0002.csv"
AUDIT_CSV = OUT / "EMBOPSA_ACQUISITION_AUDIT_V0002.csv"
...
def parse_vectors(text: str, years: np.ndarray, requested: np.ndarray, index: int, total: int) -> pd.DataFrame:
    if "$$SOE" not in text or "$$EOE" not in text:
        compact = " ".join(text.split())
        raise RuntimeError(f"REJECTED chunk {index}/{total}: Horizons data markers absent; response={compact[:800]}")
    ...
    array = array[order]
    requested_order = np.argsort(requested)
    sorted_requested = np.asarray(requested, dtype=float)[requested_order]
    sorted_years = np.asarray(years, dtype=int)[requested_order]
    if not np.allclose(array[:, 0], sorted_requested, rtol=0.0, atol=2.0e-7):
        error = float(np.max(np.abs(array[:, 0] - sorted_requested)))
        raise RuntimeError(f"REJECTED JPL epoch mismatch; maximum error={error:.12e} day")
    return pd.DataFrame({
        "astronomical_year": sorted_years,
        "jd_tdb_requested": sorted_requested,
...
def cache_valid(frame: pd.DataFrame, years: np.ndarray, jds: np.ndarray) -> bool:
    required = {"astronomical_year", "jd_tdb_requested", "jd_tdb_returned", "x_au", "y_au", "z_au", "vx_au_per_day", "vy_au_per_day", "vz_au_per_day"}
    if not required.issubset(frame.columns) or len(frame) != len(jds):
        return False
    if not np.array_equal(frame["astronomical_year"].to_numpy(dtype=int), years):
        return False
    if not np.allclose(frame["jd_tdb_requested"].to_numpy(dtype=float), jds, rtol=0.0, atol=5.0e-10):
        return False
    return bool(np.isfinite(frame[list(required - {"astronomical_year"})].to_numpy(dtype=float)).all())


def acquire_chunk(session: requests.Session, index: int, years: np.ndarray, jds: np.ndarray, total: int) -> pd.DataFrame:
    path = CACHE / f"EMBOPSA_V0002_CHUNK_{index:03d}.csv"
    if path.exists():
        try:
            cached = pd.read_csv(path)
            if cache_valid(cached, years, jds):
                print(f"CACHE CHUNK {index:03d}/{total:03d} | rows={len(cached)}")
                return cached
            print(f"REJECTED cached chunk {index:03d}/{total:03d}; reacquiring")
        except Exception as exc:
            print(f"REJECTED cached chunk {index:03d}/{total:03d}: {type(exc).__name__}; reacquiring")
    last = None
    request_params = params(jds)
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            response = session.get(API_URL, params=request_params, timeout=TIMEOUT_SECONDS)
            ...
            print(f"DEBUG retry {attempt}/{MAX_RETRIES} | chunk {index}/{total} | HTTP {status if status else 'N/A'} {type(exc).__name__}: {str(exc)[:900]}")
...
def validate(states: pd.DataFrame, normals: pd.DataFrame) -> pd.DataFrame:
    ...
    normal = normals[["normal_x", "normal_y", "normal_z"]].to_numpy(float)
    ...
    checks = {
        ...
        "finite_normal_values": bool(np.isfinite(normal).all()),
    }
...
def main() -> None:
    ...
    print("Each Horizons TLIST Julian date is individually single-quoted.")
    ...
    session.headers["User-Agent"] = "EMBOPSA-V0002/1.0"
    frames = []
    try:
